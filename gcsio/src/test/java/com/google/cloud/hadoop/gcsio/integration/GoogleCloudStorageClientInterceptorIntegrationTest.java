...
public class GoogleCloudStorageClientInterceptorIntegrationTest {
  ...
  public void testBidiReadLogs_fastFailDisabled_noGetObject() throws Exception {
    ...
    // 1. Write the file using helperGcs (Standard gRPC/JSON).
    // We do NOT use gcsImpl to write because Bidi Writes (Appendable Objects)
    // are not supported on standard regional test buckets.
    int partitionsCount = 1;
    byte[] partition =
        writeObject(helperGcs, resourceId, /* partitionSize= */ 2 * 1024 * 1024, partitionsCount);

    // Clear the logs generated by the write operation so we start fresh
    assertingHandler.flush();

    // 2. Configure the client for BIDI READS
    GoogleCloudStorageOptions storageOption =
        GCS_TRACE_OPTIONS.toBuilder()
            .setBidiEnabled(true) // <-- Enable Bidi
            .build();

    GoogleCloudStorage gcsImpl = getGCSClientImpl(storageOption);

    // 3. Read the object with Fast Fail disabled
    ...
    // 4. Verify the Telemetry Logs dynamically
    ...
    while (true) {
      try {
        ...
        // Check for ReadObject, BidiReadObject, or BidiReadObjectRanges
        if (method != null && method.contains("ReadObject")) {
          hasBidiRead = true;
        }
        i++;
      } catch (AssertionError | IndexOutOfBoundsException e) {
        // We reached the end of the captured logs! Break out of the loop.
        break;
      }
    }

    // Assert 1: We must NOT see any "GetObject" calls because we bypassed the eager fetch!
    assertThat(hasGetObject).isFalse();

    // Assert 2: We MUST see the Bidi stream initialization
