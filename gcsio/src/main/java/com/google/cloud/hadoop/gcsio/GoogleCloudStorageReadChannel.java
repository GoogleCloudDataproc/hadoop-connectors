...
public class GoogleCloudStorageReadChannel implements SeekableByteChannel {
     ...
     * @param readOptions fine-grained options specifying things like retry settings, buffering,
     *     etc. Could not be null.
    ...
    /**
     * Returns {@link GoogleCloudStorageItemInfo} used to initialize metadata in constructor or
     * {@code null} if {@link GoogleCloudStorageReadOptions#getFastFailOnNotFound()} is set to
     * {@code false}.
     */
    @Nullable
    ...
    private GoogleCloudStorageItemInfo fetchInitialMetadata() throws IOException {
        ...
        return GoogleCloudStorageItemInfo.createObject(
                ...
                /* creationTime= */ 0,
                /* modificationTime= */ 0,
                checkNotNull(object.getSize(), "size can not be null for '%s'", resourceId)
                        .longValue(),
                /* contentType= */ null,
                object.getContentEncoding(),
                /* metadata= */ null,
                checkNotNull(
                        object.getGeneration(), "generation can not be null for '%s'", resourceId),
                /* metaGeneration= */ 0,
        ...
        do {
            ...
            try {
                ...
                if (retriesAttempted != 0) {
                    logger.atInfo().log(
                            "Success after %s retries on reading '%s'",
                            retriesAttempted, resourceId);
                }
     ...
     *
     * <p>Catches and ignores all exceptions as there is not a lot the user can do to fix errors
     * here and a new connection will be needed. Especially SSLExceptions since the there's a high
     * probability that SSL connections would be broken in a way that causes {@link
     * java.nio.channels.Channel#close()} itself to throw an exception, even though underlying
        ...
        if (contentChannel != null) {
            ...
            try {
                ...
                logger.atFine().withCause(e).log(
                        "Got an exception on contentChannel.close() for '%s'; ignoring it.",
                        resourceId);
            } finally {
                contentChannel = null;
                resetContentChannel();
    ...
    protected void initMetadata(HttpHeaders headers) throws IOException {
        ...
        if (generationString == null) {
            throw new IOException(
                    String.format("Failed to retrieve generation for '%s'", resourceId));
        }
        ...
        if (resourceId.hasGenerationId()) {
            ...
            resourceId =
                    new StorageResourceId(
                            resourceId.getBucketName(), resourceId.getObjectName(), generation);
        }
    ...
    private void cacheFooter(HttpResponse response) throws IOException {
        ...
        try (InputStream footerStream = response.getContent()) {
            ...
            do {
                ...
                bytesRead =
                        footerStream.read(
                                footerContent, totalBytesRead, footerSize - totalBytesRead);
        ...
        logger.atFiner().log(
                "Prefetched %s bytes footer for '%s'", footerContent.length, resourceId);
    }
     ...
     *
     * <p>If the file encoding in GCS is gzip (and therefore the HTTP client will decompress it),
     * the entire file is always requested and we seek to the position requested. If the file
     * encoding is not gzip, only the remaining bytes to be read are requested from GCS.
     *
    ...
    protected InputStream openStream(long bytesToRead) throws IOException {
        ...
        try {
            ...
            if (!metadataInitialized
                    && errorExtractor.rangeNotSatisfiable(e)
                    && currentPosition == 0) {
        ...
        if (!metadataInitialized) {
            ...
            checkState(
                    metadataInitialized,
                    "metadata should be initialized already for '%s'",
                    resourceId);
            if (size == 0) {
                resetContentChannel();
                return new ByteArrayInputStream(new byte[0]);
     ...
     * generation, and depending on the generation read consistency setting, either retry the read
     * (of the latest generation), or handle the exception directly.
     *
    ...
    private HttpResponse handleExecuteMediaException(IOException e) throws IOException {
        ...
        String msg =
                String.format("Error reading '%s' at position %d", resourceId, currentPosition);
