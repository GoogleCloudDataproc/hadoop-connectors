...
public class GoogleCloudStorageItemInfo {
    ...
    public static GoogleCloudStorageItemInfo createObject(
        ...
        checkNotNull(resourceId, "resourceId must not be null");
        checkArgument(
                !resourceId.isRoot(), "expected object or directory but got '%s'", resourceId);
        checkArgument(
                !resourceId.isBucket(), "expected object or directory but got '%s'", resourceId);
        return new GoogleCloudStorageItemInfo(
                resourceId,
                creationTime,
                modificationTime,
    ...
    /**
     * Factory method for creating a "not found" GoogleCloudStorageItemInfo for a bucket or an
     * object.
     *
     * @param resourceId Resource ID that identifies an inferred directory
     */
     ...
     *
     * <p>Note: metadata is only supported for objects. This value is always an empty map for
     * buckets.
     */
    ...
    public boolean metadataEquals(Map<String, byte[]> otherMetadata) {
        ...
        for (Map.Entry<String, byte[]> metadataEntry : metadata.entrySet()) {
            if (!Arrays.equals(
                    metadataEntry.getValue(), otherMetadata.get(metadataEntry.getKey()))) {
                return false;
            }
        }
        return true;
        ...
        result =
                prime * result
                        + metadata.entrySet().stream()
                                .mapToInt(
                                        e ->
                                                Objects.hash(e.getKey())
                                                        + Arrays.hashCode(e.getValue()))
                                .sum();
        return result;
    }
}
