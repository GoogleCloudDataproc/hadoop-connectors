...
public class GoogleCloudStorageGrpcReadChannel implements SeekableByteChannel {
    ...
    private static GoogleCloudStorageGrpcReadChannel openChannel(
        ...
        ByteString footerContent =
                getFooterContent(resourceId, readOptions, stub, footerOffsetInBytes);
    ...
    private static ByteString getFooterContent(
        ...
        try {
            ...
            if (footerContent == null) {
                logger.atFiner().log(
                        "Prefetched footer content is null for resource '%s'", resourceId);
            } else {
                logger.atFiner().log(
    ...
    public int read(ByteBuffer byteBuffer) throws IOException {
        ...
        if ((footerContent != null) && (effectivePosition >= footerStartOffsetInBytes)) {
            logger.atFiner().log(
                    "Read request responded with footer content at position '%s'",
                    effectivePosition);
    ...
    private int readObjectContentFromGCS(ByteBuffer byteBuffer) throws IOException {
        ...
        while (moreServerContent() && byteBuffer.hasRemaining()) {
            ...
            if (bytesToSkipBeforeReading >= 0 && bytesToSkipBeforeReading < content.size()) {
                content =
                        res.getChecksummedData()
                                .getContent()
                                .substring((int) bytesToSkipBeforeReading);
                positionInGrpcStream += bytesToSkipBeforeReading;
                bytesToSkipBeforeReading = 0;
    ...
    private void validateChecksum(GetObjectMediaResponse res) throws IOException {
        ...
        int calculatedChecksum =
                Hashing.crc32c()
                        .hashBytes(res.getChecksummedData().getContent().toByteArray())
                        .asInt();
    ...
    private OptionalLong getBytesToRead(ByteBuffer byteBuffer) {
        ...
        if (readStrategy == Fadvise.RANDOM) {
            ...
            optionalBytesToRead =
                    OptionalLong.of(max((long) byteBuffer.remaining(), rangeRequestSize));
        }
                        ...
                        try {
                            ...
                            StorageBlockingStub blockingStub =
                                    stub.withDeadlineAfter(
                                            readOptions.getGrpcReadTimeoutMillis(), MILLISECONDS);
                            try {
