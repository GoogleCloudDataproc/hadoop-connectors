...
public class StorageResourceId {
    ...
    /**
     * Constructor for a StorageResourceId representing a Bucket; {@code getObjectName()} will
     * return null for a StorageResourceId that represents a Bucket.
     *
    ...
    public StorageResourceId toDirectoryId() {
        if (isStorageObject() && !StringPaths.isDirectoryPath(getObjectName())) {
            return new StorageResourceId(
                    getBucketName(), StringPaths.toDirectoryPath(getObjectName()));
        }
        return this;
    }
