...
public interface GoogleCloudStorage {
     ...
     *
     * <p>GCS does not implement full concept of file system paths but it does expose some notion of
     * a delimiter that can be used with ${code Storage.Objects.List} to control which items are
     * listed.
     */
     ...
     * single-item version of this method. Implementations may use different flow than the
     * single-item version for greater efficiency. See {@link #create(StorageResourceId)} for the
     * behavior if StorageResourceId.getGenerationId() is explicitly set.
     */
    ...
    /**
     * Copies metadata of the given objects. After the copy is successfully complete, each object
     * blob is reachable by two different names. Copying between two different locations or between
     * two different storage classes is not allowed.
     *
     ...
     *
     * <ul>
     *   <li/>With {@code prefix == null} and {@code delimiter == /}, we get: {@code d1/}, {@code
     *       o1}
    ...
    default ListPage<GoogleCloudStorageItemInfo> listObjectInfoPage(
        ...
        return listObjectInfoPage(
                bucketName, objectNamePrefix, ListObjectOptions.DEFAULT, pageToken);
    }
    /**
     * The same semantics as {@link #listObjectInfo}, but returns only result of single list request
     * (1 page).
    ...
    /**
     * Composes inputs into a single GCS object. This performs a GCS Compose. Objects will be
     * composed according to the order they appear in the input. The destination object, if already
     * present, will be overwritten. Sources and destination are assumed to be in the same bucket.
     *
    ...
    /**
     * Composes inputs into a single GCS object. This performs a GCS Compose. Objects will be
     * composed according to the order they appear in the input. The destination object will have
     * metadata set according to {@code options}. Overwrite semantics for the destination object
     * will follow the same semantics as {@link #create(StorageResourceId, CreateObjectOptions)}.
     * See {@link #create(StorageResourceId)} for the behavior if
     * StorageResourceId.getGenerationId() is explicitly set. The bucket must be the same for all
