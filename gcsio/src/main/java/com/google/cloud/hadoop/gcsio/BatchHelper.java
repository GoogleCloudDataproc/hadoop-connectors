...
public class BatchHelper {
  ...
  public static class Factory {
    ...
    BatchHelper newBatchHelper(
      ...
      int numThreads = (int) Math.ceil((double) totalRequests / requestsPerBatch);
  ...
  private static ExecutorService newRequestsExecutor(int numThreads) {
    ThreadPoolExecutor requestsExecutor =
        new ThreadPoolExecutor(
            ...
            /* keepAliveTime= */ 5L, TimeUnit.SECONDS,
            new LinkedBlockingQueue<Runnable>(numThreads * 20),
            THREAD_FACTORY);
