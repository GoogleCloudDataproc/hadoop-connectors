...
export GCS_TEST_JSON_KEYFILE=$3

print_usage() {
  echo -n "$0 (hadoop2 | hadoop3) <project_id> <path_to_json_keyfile> [optional_maven_parameters, [...]]"
}
...
check_required_params() {
  ...
  check_required_param "${GCS_TEST_JSON_KEYFILE}" "Keyfile is required."

  if [[ ! -f "${GCS_TEST_JSON_KEYFILE}" ]]; then
    echo "Can't find private key file ${GCS_TEST_JSON_KEYFILE}"
    print_Usage
    exit 1
  fi
}
...
if [[ ! -f $GCS_TEST_JSON_KEYFILE ]]; then
  echo "Resolved '${GCS_TEST_JSON_KEYFILE}' keyfile does not exist."
  exit 1
fi
...
./mvnw -B -e -T1C "-P${HADOOP_VERSION}" -Pintegration-test clean verify "${@:4}"
