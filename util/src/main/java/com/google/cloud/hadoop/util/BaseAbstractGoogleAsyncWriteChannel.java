...
public abstract class BaseAbstractGoogleAsyncWriteChannel<T> implements WritableByteChannel {
  ...
  // write channel (pipeSink below). The pipe is formed by connecting pipeSink to pipeSource
  ...
  private InputStream initializeUploadPipe() throws IOException {
    switch (pipeType) {
      case NIO_CHANNEL_PIPE:
        ...
        return pipeBufferSize > 0
            ? new BufferedInputStream(pipeSource, pipeBufferSize)
            : pipeSource;
      case IO_STREAM_PIPE:
        PipedInputStream internalPipeSource = new PipedInputStream(pipeBufferSize);
    ...
    throw new IllegalStateException("Unknown PipeType: " + pipeType);
  }
